...
function PGF.Dialog_Reset()
    ...
    PGF.Dialog_ResetGenericField(dialog, "Tankrole")
    PGF.Dialog_ResetGenericField(dialog, "Healrole")
    PGF.Dialog_ResetGenericField(dialog, "Dpsrole")
...
function PGF.Dialog_StartSearch()
    PremadeGroupsFilterDialog.RefreshButton:SetText(L["dialog.searching"])
    PremadeGroupsFilterDialog.searching = true
    PremadeGroupsFilterDialog.TimeSinceLastUpdate = 0
    LFGListSearchPanel_DoSearch(LFGListFrame.SearchPanel)
end

function PGF.Dialog_StopSearch()
    PremadeGroupsFilterDialog.RefreshButton:SetText(L["dialog.refresh"])
    PremadeGroupsFilterDialog.searching = false
end
...
function PGF.OnLFGListFrameSetActivePanel(self, panel)
    ...
    if not buttonHooksInitialized and panel == self.SearchPanel then
        ...
        for i = 1, #buttons do
            ...
            buttons[i]:HookScript("OnClick", PGF.OnLFGListSearchEntryOnClick)
        end
    end
end
